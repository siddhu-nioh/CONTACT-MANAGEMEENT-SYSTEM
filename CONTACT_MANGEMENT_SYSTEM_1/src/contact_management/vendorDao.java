...
public class vendorDao {
	...
	public boolean addContact(vendor b) {
		boolean f = false;
		try {
			String query = "insert into vendor(s_no,name,organization_name,phone_number,mail_id,Designation,adress,task_meetingDate) values(?,?,?,?,?,?,?,?)";
	...
	public boolean updateContact(vendor b) {
		boolean f = false;
		try {
			PreparedStatement ps = conn.prepareStatement(
					"update vendor set name=?,organization_name=?,phone_number=?,mail_id=?,task_meetingDate=? where s_no=?");
	...
	public boolean deleteContact(int s_no) {
		boolean f = false;
		try {
			PreparedStatement ps = conn.prepareStatement("delete from books where s_no=?");
			ps.setInt(1, s_no);
			int i = ps.executeUpdate();
			if (i >= 1) {
				f = true;
	...
	public vendor searchContact(int s_no) {
		vendor b = new vendor();
		try {
			PreparedStatement ps = conn.prepareStatement("select *from vendor where s_no=?");
			ps.setInt(1, s_no);
	...
	public vendor searchTask( int s_no) {
		vendor b = new vendor();
		try {
			PreparedStatement ps = conn.prepareStatement("select *from vendor where s_no=?");
			ps.setInt(1, s_no);
		...
		return b;}
		public vendor searchRemind( int s_no) {
			vendor b = new vendor();
			try {
				PreparedStatement ps = conn.prepareStatement("select *from vendor where s_no=?");
				ps.setInt(1, s_no);
	...
	public List<vendor> getAllContacts() {
...
//	public vendor taskFollowUp(int s_no) throws Exception {
...
//		PreparedStatement ps = conn.prepareStatement("update vendor set task_reminder+=1 where s_no=?");
//		vendor b = null;
...
//		}
//		vendor st = dao.searchContact(s_no);
//		if (st == null) {
